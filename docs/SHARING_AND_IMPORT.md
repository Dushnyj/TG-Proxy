...
1. Откройте **Настройки → Relay**.
2. Выберите нужное сохранённое подключение.
3. Нажмите **Поделиться Relay**.
4. Выберите один способ:
   - скопировать кликабельную ссылку;
   - показать QR-код;
   - открыть системное меню **Поделиться**;
   - сохранить файл подключения.
...
Приложение не применяет внешнее подключение без подтверждения. После импорта его можно
проверить кнопкой **Тест** и выбрать основным или резервным для профиля.

## Импорт по QR-коду

1. На телефоне владельца откройте QR-код Relay.
2. На телефоне получателя откройте импорт/сканирование QR в TG Proxy.
