...
## Передача Relay и перенос профиля
...
Во вкладке **Настройки → Подключение → Перенос профиля подключения** можно перенести только
выбранный профиль. Экспорт без секретов не содержит MTProto secret и VPS Relay. Защищённый
паролем экспорт содержит secret и одно основное Relay-подключение выбранного профиля. Это не
резервная копия всего приложения: пул Relay, привязки остальных профилей, owner/SSH/DuckDNS,
интерфейсные и системные параметры в файл не входят.

Перенос профиля не заменяет кнопку **Поделиться**: первый сценарий нужен владельцу для переноса
одного профиля, второй — получателю одного выбранного client token.
...
На новом экране ручное добавление и импорт выполняют сетевую проверку до сохранения либо по
явному выбору пользователя сохраняются без проверки. Полная проверка показывает каждый
production/test DC отдельно для main и media, текущую попытку и итог каждого маршрута.
